...
其主要用于 ` 独立应用 ` 的进程化管理，在 Node.js 单机服务部署方面笔记适合。可以用于生产环境下启动同个应用的多个实例提高 CPU 利用率、抗风险、热加载等能力。
...
const threadPool = new DynamicThreadPool({
  execFunction: (value) => `rsp:${value}`,
});
const executor = threadPool.createExecutor();

threadPool.exec('test');
executor
  .setExecFunction((value) => `rsp:${value}`)
  .setExecString(`module.exports = (value) => value`)
