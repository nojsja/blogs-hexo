...
- [IV. VSCode 插件系统及其生态](#iv-vscode-%E6%8F%92%E4%BB%B6%E7%B3%BB%E7%BB%9F%E5%8F%8A%E5%85%B6%E7%94%9F%E6%80%81)
  - [一、VSCode 整体架构](#%E4%B8%80vscode-%E6%95%B4%E4%BD%93%E6%9E%B6%E6%9E%84)
  - [二、VSCode 插件开发简介](#%E4%BA%8Cvscode-%E6%8F%92%E4%BB%B6%E5%BC%80%E5%8F%91%E7%AE%80%E4%BB%8B)
...
很多时候插件直接作为一个可执行文件提供给主程序调用者，已经作为一个独立编译完成的应用，一般这类插件是主程序需要通过子进程的方式运行，因此无法直接利用宿主环境 API 调用来实现功能。

在子进程创建时，可以通过命令行参数来实现初始化参数的传递。如：

```bash
$: ls -alh
```

如果需要和子进程进行持续的双向数据传递，只能通过进程IPC通信来实现。

Node.js 程序通过 child_process 模块提供了衍生子进程的能力，child_process 提供多种子进程的创建方式：

- spawn 创建新进程，执行结果已流的形式返回，只能通过事件来获取结果数据，操作麻烦。
...
- execFile 创建新进程，按照其后面的File名字，执行一个可执行文件，可以带选项，以回调形式返回调用结果，可以得到完整数据，方便了很多。
...
- exec  创建新进程，可以直接执行shell命令，简化了shell命令执行方式，执行结果以回调方式返回。
...
其中，spawn是所有方法的基础，exec底层是调用了execFile。

#### 3. 题外话：跨应用间的通信

跨应用间的通信可以使用成熟的通信协议，比如：HTTP、RPC（Remote Procedure Call）远程过程调用协议、Websocket等，他们的特点各有不同，可以根据实际情况选择合适的通信协议：

- HTTP：简单，易于实现，但是不安全，无状态。
- RPC：适用于 IO 密集的内网服务调用，也适用于服务调用过于密集和复杂的场景。
- Websockt：全双工通信，适用于实时通信场景。

一个例子就是自用的 Joplin 笔记软件的网页剪切功能，笔记软件和浏览器是独立的两个应用。笔记安装在PC端，启动时会在某个端口启动一个HTTP服务，Joplin的网页剪贴插件通过和固定的本机 Joplin 服务端口通信，将捕获到的网页数据转换成 Markdown 然后通过HTTP协议发送给笔记软件进行存储。
...
Webpack 作为一个静态资源打包器会将代码和静态文件打包成一个或多个bundle文件，这些bundle文件即主程序的输出结果。

Webpack 构建工具使用 Plugin 来处理 Webpack 内部的构建资源，在构建过程中，Webpack 会广播出许多事件，插件可以监听这些事件，在合适的时机通过 Webpack 提供的 API 改变输出结果。

#### 2. 扩展主程序 UI 功能

Vscode 插件可以扩展编辑器的功能，比如：代码提示、代码格式化、代码片段、取色板、项目路径管理等。

#### 3. 统计和分析主程序的运行指标信息

某些插件不直接显式作用于主程序，而是使用主程序的环境API监控和统计其运行情况。比如之前做的一个 Electron 进程管理工具，它的设计目的之一就是无感知的统计一个 Electron 桌面应用中的所有进程的资源占用和运行情况，并提供UI界面以便于开发者快速定位和解决应用中的性能问题。
