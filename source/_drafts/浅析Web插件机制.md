...
- [III. 插件机制的核心关注点](#iii-%E6%8F%92%E4%BB%B6%E6%9C%BA%E5%88%B6%E7%9A%84%E6%A0%B8%E5%BF%83%E5%85%B3%E6%B3%A8%E7%82%B9)
  - [一、「接口」：插件如何在特定时机和位置接入](#%E4%B8%80%E6%8E%A5%E5%8F%A3%E6%8F%92%E4%BB%B6%E5%A6%82%E4%BD%95%E5%9C%A8%E7%89%B9%E5%AE%9A%E6%97%B6%E6%9C%BA%E5%92%8C%E4%BD%8D%E7%BD%AE%E6%8E%A5%E5%85%A5)
    ...
    - [接入位置](#%E6%8E%A5%E5%85%A5%E4%BD%8D%E7%BD%AE)
...
插件程序作为主程序的功能扩展，需要在主程序运行过程中的特定时机和位置进行接入，当然接入后也能通过某种机制从运行环境中移除。

这里提及了 **时机** 和 **位置** 两个关键词，从这两个指标来看，插件接入的位置可以反应一个应用程序的功能模块耦合度，而通过调用时机则能度量其运行性能。
...
而另一方面，插件的存放位置也有一定考究。如果插件以源代码的方式提供的话，那位置应该是主程序构建前的构建依赖目录中，比如 node_modules 目录也可以视为插件目录，这类插件服务于程序开发和构建阶段，大部分面向程序开发者而不是用户。如果插件位置是程序安装后的源代码目录或是本地硬盘自定义目录中，这类插件多是以二进制可执行文件或是可执行代码的压缩包格式，主程序使用他们时需要使用子进程来加载插件或是直接解压插件源代码到主程序源代码目录。

（举例 7zip 使用 asar 插件解压 asar 压缩包）
